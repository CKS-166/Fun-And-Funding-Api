...
namespace Fun_Funding.Application.Services.ExternalServices
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendReportAsync(string toEmail, string projectName, string userName, DateTime reportedDate, string reporter, string reason)
        {
            string body = string.Empty;
            await _fluentEmail
                .To(toEmail)
                .Subject($@"[WARNNG] - PROJECT: {projectName} HAS BEEN REPORTED")
                .Body(GenerateReportedEmailBody(projectName, userName, reportedDate, reporter, reason), isHtml: true)
                .SendAsync();
        }
        ...
        private string GenerateReportedEmailBody(string projectName, string userName, DateTime reportedDate, string reporter, string reason)
        {
            return $@"
                <!DOCTYPE html>
<html lang=""en"">
